...
def _write_flat_dataset(src_path, dst_path, src_file, episodes, merge_gripper=True):
...
def convert_dataset(src_path, dst_path, merge_gripper=True):
    ...
    with h5py.File(src_path, "r") as src_file:
        ...
        _write_flat_dataset(
            ...
            src_file=src_file,
            episodes=episodes,
            merge_gripper=merge_gripper,
        )


def convert_dataset_sharded(src_path, dst_dir, num_shards, merge_gripper=True):
    src_path = Path(src_path)
    ...
    with h5py.File(src_path, "r") as src_file:
        ...
        shard_slices = np.array_split(np.arange(len(episodes)), num_shards)
        shard_name = src_path.stem

        for shard_idx, shard_episode_idx in enumerate(shard_slices):
            if len(shard_episode_idx) == 0:
                continue
            shard_episodes = [episodes[int(i)] for i in shard_episode_idx]
            shard_path = dst_dir / f"{shard_name}_shard_{shard_idx:03d}.h5"
            _write_flat_dataset(
                src_path=src_path,
                dst_path=shard_path,
                src_file=src_file,
